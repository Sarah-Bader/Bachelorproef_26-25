...
Het doel van dit onderzoek is het ontwikkelen van een proof-of-concept (PoC) die een representatieve subset van HLASM-mnemonics en labels statisch analyseert en deze omzet in een visueel control-flowdiagram in Mermaid.js-syntax.
...
\begin{itemize}
    ...
    \item Welke HLASM-mnemonics en labels bepalen de control flow in een programma?
\end{itemize}
...
Een HLASM-programma bestaat uit één of meerdere modules. Elke module wordt samengesteld uit één broncodebestand, de bijbehorende copybooks en macro's. De structuur van HLASM-code wordt bepaald door mnemonics en labels. Om een subroutine aan te roepen, kun je gebruikmaken van mnemonics zoals BAL of BAS. Deze mnemonics slaan het retouradres (het adres van de instructie na de call) op in een register. Om terug te keren vanuit een subroutine kun je gebruikmaken van de mnemonic BR (Branch to Register), met deze mnemonic spring je naar het adres in het meegegeven register \autocite{ward_2013}.
...
Het resultaat van deze fase is een theoretisch onderzoek naar de probleemstelling en oplossingsdomein, en een afgebakende scope van HLASM-mnemonics en labels voor de verdere ontwikkeling van de technische oplossing.

\subsection{Fase 2: Implementatie en Evaluatie}

Tijdens de implementatie- en evaluatiefase wordt de proof-of-concept uitgewerkt en iteratief geëvalueerd. De ontwikkeling gebeurt volgens een scrum-werkwijze om de effectiviteit van de PoC te waarborgen. Met behulp van de subset die in de vorige fase werd gedefinieerd, wordt een Python-gebaseerde parser ontwikkeld die HLASM-code statisch analyseert. De parser identificeert mnemonics en labels aan de hand van patroonherkenning in de structuur van HLASM-broncode en zet deze via een mapping om naar Mermaid.js-syntax.
...
\begin{itemize}
    ...
    \item een mapping van de geselecteerde HLASM- en labels naar Mermaid.js-elementen.
\end{itemize}
